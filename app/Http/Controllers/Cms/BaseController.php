...
class BaseController extends Controller
{
    ...
    public function handleImageCropUpload($record,$urlColumn,$diff,$imageFile,$column,$width,$height,$w,$h,$x,$y){
        ...
        if($record->$column){
            File::delete(public_path($record->$column));
        }
        $keyString = self::seo_friendly_url($record->$urlColumn);
        if(strlen($diff)>0)
            $filename = '/storage/img/pages/'.$record->imageFileName.'/'.$diff.'/'.$keyString.'_'.time();
        else
            $filename = '/storage/img/pages/'.$record->imageFileName.'/'.$keyString.'_'.time();
        ...
        $image->crop($w, $h, $x, $y)->resize($width,$height)->save(public_path($finalName));
    ...
    public function handleDestroy($model, $record, $urlColumn, $gallery = true, $materials = false){
        if($gallery){
            if($record->images){
                foreach($record->images as $image) {
                    File::delete(public_path($image->main_image));
                    $image->delete();
                }
            }
        }
        if($materials){
            if($record->materials){
                foreach($record->materials as $material) {
                    File::delete(public_path($material->main_file));
                    $material->delete();
                }
            }
        }
        if($record){
            if(count($model::$imageFieldNames)){
                foreach($model::$imageFieldNames as $imageField){
                    File::delete(public_path($record->$imageField));
                }
            }
            if(count($model::$docFields)){
                foreach($model::$docFields as $docField){
                    File::delete(public_path($record->$docField));
                }
            }
